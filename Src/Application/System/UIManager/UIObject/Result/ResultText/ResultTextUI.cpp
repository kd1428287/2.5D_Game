#include "ResultTextUI.h"

void ResultTextUI::Init()
{
	m_time = 0.f;
	m_pos = m_basePos;
	m_isPressed = true;
	...
	m_startSub = GLOBALEVENT.subscribe<Events::Else::ResultToTitleBegin>
		([this](const Events::Else::ResultToTitleBegin& e)
			{
				m_isPressed = true;
			}
		);
...
void ResultTextUI::Update(float dt)
{
	m_time += dt;
